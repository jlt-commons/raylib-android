...
**Most of these were found on iOS**, by
[raylib-ios](https://github.com/jlt-commons/raylib-ios), the sibling this tree
was ported from. They are kept because a finding does not stop being true when
the host changes, and because rediscovering them would be a waste. Which ones
still bear on an Android build:

| finding | still live here |
| --- | --- |
| `sa-os-family` calls a native iOS build Linux | no — `tarm64le` on Android *is* Linux as far as jolt is concerned, and that is the right answer |
| `os.name` says "Mac OS X" on an iPhone | no — the same code path answers "Linux" here, which is what makes jolt.nrepl's sockets work |
| `CUSTOMIZE_BUILD` reads a disabled option as ON | yes, if you ever pass it — this build does not, which is why the Android CMake has no `SUPPORT_*` flags at all |
| `GetTouchPointId` truncates `SDL_FingerID` | no — that is the SDL platform layer; `rcore_android.c` uses `AMotionEvent_getPointerId`, which is already an `int32_t` |
| `FLAG_WINDOW_HIGHDPI` is honoured and then discarded | no — the Android backend sizes the screen from the native window, and this host passes no flags |
| `GetFPS` is stateful and nothing says so | **yes** — it is in `rcore.c`, shared by every platform |
| raylib does not draw into framebuffer 0 on iOS | no, and it never was a bug: on Android framebuffer 0 is the EGL window surface and the question does not arise |
...
`// Get current FPS`. On an iPhone 17 Pro running a build linked against master,
one binary, one variable, the call cadence:
