...
answer in `last-fps`, so frame rate becomes a value to read rather than a log
line to scrape. `raylib.gallery/tap!` opens a scene without a finger.
...
from a `--dev` build, where a redefined `trail-length` takes effect on the next
frame; here that is what `jolt build-app` gives you by default, and only a
release build drops it. The cost is negligible: `--dev` held mean 17.05 ms
