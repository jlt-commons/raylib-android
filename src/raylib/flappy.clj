(ns raylib.flappy
  ...
  unchanged, under this owner loop. The host hands this one input snapshot and
  a delta per frame (RAY-018's contract); it draws what comes back.

  One entry namespace per APK — see the export at the bottom.
  `NS=raylib.flappy jolt build-app` builds this one; examples/gallery draws the
  same scene through the contract's draw-scene!."
...
(defn- frame [{:keys [touches game] :as s}]
  (let [m     (metrics)
    ...
    (when (= :press (get-in input [:pointer :phase]))
      (rl/log "flappy: flap at frame" (:frame s') "score" (:score game') (if (:over? game) "(restart)" "")))
    (when (and (:over? game') (not (:over? game)))
      (rl/log "flappy: game over at frame" (:frame s') "score" (:score game')
              "elapsed" (format "%.1f" (:elapsed game')) "s"))
    (when (rl/back-pressed?)
      (rl/log "flappy: Back — finishing")
      (rl/finish!))
    (draw! s' m)
    s'))
