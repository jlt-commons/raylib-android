(ns raylib.gallery
  "examples/gallery: the Android experiment's scene contract on this host.
  ...
  One entry namespace per APK — see the export at the bottom. This is what a
  release build ships: `jolt release` with no NS builds it. A debug build
  defaults to raylib.live instead, which is this plus an nREPL."
...
(defn tap!
  "Queue a synthetic tap at [x y] in screen pixels, which is the only unit
  raylib deals in here. The next frame sees a press there and the frame after
  sees the release, which is the edge the gallery opens a card on."
  [x y]
...
(defn- below-the-inset
  "Their layout, computed for the screen minus the top inset and lowered by it.

  On Android that inset is 0 and this is the identity, because the manifest
...
(defn- frame [{:keys [k top touches gstate] :as s}]
  (let [category (:category s)
        ...
        layout   (below-the-inset m sizes top ids)
        ...
        list-back? (and (not scene?) category
                        (or hw-back? (and press? (within? (:back layout) point))))
        quit?      (and (not scene?) (nil? category) hw-back?)
        ;; :back? is what the pure contract reads, and at gallery level it
        ;; means close-requested?. Only a scene's Back is passed through, so
        ;; the two Back-shaped things above cannot also request closure.
        input    (assoc input :delta-seconds (rl/get-frame-time)
                        :back? (or (= hit :back) (and hw-back? scene?)))
        ...
        gstate  (let [events (:scene-events gstate)]
                  (doseq [e events] (rl/log "gallery:" (pr-str e)))
                  (assoc gstate :scene-events []))
        gstate  (if (:close-requested? gstate)
                  (do (rl/log "gallery: a scene requested closure — finishing")
                      (rl/finish!)
                      (assoc gstate :close-requested? false))
                  gstate)]
    (when quit?
      (rl/log "gallery: Back at the top level — finishing")
      (rl/finish!))
...
;; build, so the name is there before jolt_library_init returns. raylib.live
;; requires this namespace and adds "raylib_main_debug" beside it; a release
;; image built from here carries only this one, which main.c asserts.
